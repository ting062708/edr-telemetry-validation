...
run   – Discover sample, deliver to VM, capture stdout, then immediately
        ...
        python telemetry_runner.py run \\
            --case REG-MODIFY-001 \\
            --json  E:\\EDR\\EDRTest\\log\\Registry\\json\\export.json \\
...
_RESULTS = _ROOT / 'results' / 'match_history'
...
def _locate_sample_exe(test_case: dict, vm_config: dict):
    ...
    root = Path(root_cfg) if root_cfg else (_ROOT.parent.parent / 'EDRTest' / 'samples')
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else (
        _ROOT / 'runs' / test_case.get('module', 'Unknown') / test_case['id']
    )
    out_dir.mkdir(parents=True, exist_ok=True)
