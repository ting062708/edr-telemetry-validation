...
1. **变体分组落地（第十节）**：`capKeyOf()` 按 case_id 去末尾序号分组；矩阵能力行
   ...
   任一变体采到→采集 / 全缺→缺失 / 有变体未匹配(无采集)→疑问 / 单case待测→待测。
...
- **能力级**（同一能力的变体组）：用并集判定（任一变体采到 = 能力「对」）
- **单个变体**：必须显示自己的真实状态——没跑的变体显示「待判定」，**不能继承能力级的并集判定**
- 例如：PROC-IMAGE-LOAD-001（采集通过）+ PROC-IMAGE-LOAD-002（没跑）→ 能力级「采集通过」，但 -002 变体显示「待判定」
