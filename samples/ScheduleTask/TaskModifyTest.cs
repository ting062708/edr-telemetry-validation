...
namespace TaskModifyTest
{
    internal class Program
    {
        private const string TaskName = "EDRTelemetryTaskModify";
        private const string XmlPath = @"C:\EDRTest\samples\ScheduleTask\EDRTelemetryTaskModify.xml";

        private static string BuildXml(string arg)
        {
            return "<?xml version=\"1.0\" encoding=\"UTF-16\"?>" +
                "<Task version=\"1.2\" xmlns=\"http://schemas.microsoft.com/windows/2004/02/mit/task\">" +
                "  <Triggers><TimeTrigger><StartBoundary>2026-01-01T00:00:00</StartBoundary></TimeTrigger></Triggers>" +
                "  <Actions><Exec><Command>cmd.exe</Command><Arguments>/c " + arg + "</Arguments></Exec></Actions>" +
        ...
        private static int Main(string[] args)
        {
            ...
            if (caseId != "TASK-MODIFY-002")
            {
                Console.Error.WriteLine("Usage: TaskModifyTest.exe --case TASK-MODIFY-002");
                return 2;
            }
            ...
            Write("[META] TestCaseID=" + caseId + " Module=ScheduleTask Action=SchedTaskUpdateXml");
            Write("[META] Process=TaskModifyTest.exe PID=" + Process.GetCurrentProcess().Id + " Hostname=" + Environment.MachineName);

            try
            {
                Write("[SETUP-BEGIN] TimeUTC=" + UtcNow());
                RunCmd("schtasks /delete /tn " + TaskName + " /f");
                File.WriteAllText(XmlPath, BuildXml("exit"), System.Text.Encoding.Unicode);
                RunCmd("schtasks /create /tn " + TaskName + " /xml " + XmlPath + " /f");
                Write("[SETUP] TaskCreated=true");
                Write("[SETUP-END] TimeUTC=" + UtcNow());

                Write("[TARGET-BEGIN] TimeUTC=" + UtcNow());
                Write("[TARGET] Operation=SchedTaskUpdate");
                Write("[TARGET] TaskName=" + TaskName);
                Write("[TARGET] Method=XMLReimport");

                File.WriteAllText(XmlPath, BuildXml("echo modified"), System.Text.Encoding.Unicode);
                RunCmd("schtasks /create /tn " + TaskName + " /xml " + XmlPath + " /f");
