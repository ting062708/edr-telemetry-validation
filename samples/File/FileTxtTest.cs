...
namespace FileTxtTest
{
    internal class Program
    {
        private const string TestDir = @"C:\Temp\EDRTest";
        private const string TxtFileName = "EDRTelemetryFileTest.txt";
        private const string TxtContent = "EDRTelemetryFileTest: plain text file create";
        ...
        private static int Main(string[] args)
        {
            ...
            if (caseId != "FILE-CREATE-003")
            {
                Console.Error.WriteLine("Usage: FileTxtTest.exe --case FILE-CREATE-003");
                return 2;
            }
            ...
            Write("[META] TestCaseID=" + caseId + " Module=File Action=FileCreateTxt");
            Write("[META] Process=FileTxtTest.exe PID=" + Process.GetCurrentProcess().Id + " Hostname=" + Environment.MachineName);

            try
            {
                Write("[SETUP-BEGIN] TimeUTC=" + UtcNow());
                Directory.CreateDirectory(TestDir);
                string fullPath = Path.Combine(TestDir, TxtFileName);
                if (File.Exists(fullPath)) File.Delete(fullPath);
                Write("[SETUP] TargetPath=" + fullPath);
                Write("[SETUP] FileType=txt");
                ...
                Write("[TARGET] FileType=txt");

                File.WriteAllText(fullPath, TxtContent);
                System.Threading.Thread.Sleep(500);
                if (!File.Exists(fullPath)) throw new IOException("TXT file was not created: " + fullPath);
