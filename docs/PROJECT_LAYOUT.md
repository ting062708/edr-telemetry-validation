...
| 目录 | 作用 | 谁读写 |
|---|---|---|
| `docs/` | 文档中心：AI 总纲 / 行为链 / 规范 / IOA 实例 / 方法论 | 人 |
| `automation/` | 自动化工具链（下节 config/core/runner/tools 均在此） | 代码 |
| `samples/` | 测试样本源码（按能力划分，15 模块） | 人改源码 |
| `frontend/` | Web 监控前端（server.py + html） | 代码 |
| `results/` | 结果归档：总览 / 分模块 / 历史 / match_history + runs/ | 代码写，人看 |
| `logs/` | 导出的 IOA 日志（json） | 人导，代码读 |
| `tools/` | 第三方工具（Sysmon） | — |
...
```
改样本(源码 samples\<Module>\) → 编译 → samples\<Module>\<Module>Test.exe
   ↓ automation\runner\telemetry_runner.py run --case <CASE-ID>
   ↓ automation\core\deliverer.py 投递 VM 执行 → 捕获 stdout → results\runs\<Module>\<CASE>\<case>_stdout.txt
   ↓ 导出 Sysmon evtx → results\runs\<Module>\<CASE>\sysmon_<case>.evtx
   ↓ 人导 IOA 日志 → logs\<模块大写>\json\*.json
   ↓ runner match → core\matcher.py + core\normalizer.py + core\stdout_parser.py
   ↓ core\verdict.py 判定 → results\runs\<Module>\<CASE>\match_result.json
...
## 四、已完成的目录重构（2026-09-01）

- docs/ 集中所有 .md；automation/ 只留脚本与配置；samples/ 按能力划分（15 模块扁平化）
- 日志三处合一为 logs/（探测链改单点）；results/ 归档 + runs/ 运行产物
- 删除 EDRTelemetry / EDRTest 旧结构及版本副本；命名统一 Net→Network、process→Process
- 补顶层 README.md 作仓库门面
