...
namespace FileLifecycleTest
{
    internal class Program
    {
        ...
        private const string TextName = "FileLifecycle_Target.txt";
        private const string DllName = "FileLifecycle_Target.dll";
        ...
        private static int Main(string[] args)
        {
            ...
            string textPath = Path.Combine(baseDir, TextName);
            ...
            {
                WriteSetupBegin();
                PrepareCase(caseId, originalPath, renamedPath, textPath, dllPath);
                WriteSetupEnd();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                WriteTargetBegin();
                targetStarted = true;
                ExecuteTarget(caseId, originalPath, renamedPath, textPath, dllPath);
                WriteTargetEnd();
                WriteResult(true, 0);
                WriteRunEnd(runId);
                return 0;
        ...
        private static void PrepareCase(string caseId, string originalPath, string renamedPath, string textPath, string dllPath)
        {
            ...
            RemoveIfExists(textPath, "StaleTextRemoved");
            RemoveIfExists(dllPath, "StaleDllRemoved");
            ...
            switch (caseId)
            {
                ...
                case "FILE-OPEN-001":
                    // Industry-standard: read the same .exe baseline as other cases.
                ...
                case "FILE-DELETE-001":
                    // IOA collects .dll deletes but not .txt/.json; delete a .dll baseline.
                    CreateBaselineFile(dllPath, caseId);
                    Console.WriteLine("[SETUP] BaselineCreated=true");
                    Console.WriteLine("[SETUP] TargetPath=" + dllPath);
                    break;
        ...
        private static void ExecuteTarget(string caseId, string originalPath, string renamedPath, string textPath, string dllPath)
        {
            switch (caseId)
            {
                case "FILE-CREATE-001":
        ...
        {
            // Industry-standard: create a file AND write real content (non-empty).
            string content = "EDR_FILE_CREATE_TEST\r\nCreatedUTC=" + UtcNow() + "\r\n";
            ...
            Console.WriteLine("[TARGET] FileName=" + Path.GetFileName(path));
            Console.WriteLine("[TARGET] Operation=Create");
            Console.WriteLine("[TARGET] Size=" + new FileInfo(path).Length);
            Console.WriteLine("[TARGET] ContentMd5=" + Md5Hex(path));
        }
        ...
        {
            // Industry-standard: delete without artificial sleep.
            File.Delete(path);
            if (File.Exists(path))
        ...
        private static void RunModify(string path)
        {
            ...
            Console.WriteLine("[TARGET] BytesAppended=" + (afterSize - beforeSize));
            Console.WriteLine("[TARGET] ContentMd5=" + Md5Hex(path));
        }
        ...
        private static void RunRename(string sourcePath, string destinationPath)
        {
            ...
            Console.WriteLine("[TARGET] SourceFileName=" + Path.GetFileName(sourcePath));
            Console.WriteLine("[TARGET] DestinationFileName=" + Path.GetFileName(destinationPath));
            Console.WriteLine("[TARGET] Operation=Rename");
            Console.WriteLine("[TARGET] ContentMd5=" + Md5Hex(destinationPath));
        }
