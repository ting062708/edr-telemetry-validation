...
namespace AccountLifecycleTest
{
    internal class Program
    {
        ...
        private const string ModifiedPassword = "EdrT3st!2026_Bb";
            ...
            try
            {
                ...
                switch (testCase.Id)
                {
                    case "ACCOUNT-CREATE-001":
                        ...
                        Console.WriteLine("[TARGET] SamAccountName=" + UserName);
                        Console.WriteLine("[TARGET] Comment=" + CreatedComment);
                        Console.WriteLine("[TARGET] Flags=0x" + (UF_SCRIPT | UF_NORMAL_ACCOUNT).ToString("X"));
                        // Give lsass time to flush the 4720 (AccountCreate) event before
                        // cleanup deletes the account, otherwise the create-then-delete
                        // in <100ms gets coalesced and IOA never records AccountCreate.
                        Thread.Sleep(5000);
                        break;
                    ...
                    case "ACCOUNT-MODIFY-001":
                        ...
                        Console.WriteLine("[TARGET] Field=Password");
                        Console.WriteLine("[TARGET] NewValue=" + ModifiedPassword);
                        // Same reason as CREATE: let the 4724 (AccountPwdReset) event
                        // flush before cleanup deletes the account.
                        Thread.Sleep(5000);
                        break;
        ...
        {
            USER_INFO_1003 modifyInfo = new USER_INFO_1003
            {
                usri1003_password = ModifiedPassword
            };

            uint parameterError;
            uint status = NetUserSetInfo(null, UserName, 1003, ref modifyInfo, out parameterError);
            if (status != NERR_SUCCESS)
            {
                throw new InvalidOperationException(
